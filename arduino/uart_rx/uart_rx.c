...
void uart_rx_init(PIO pio, uint pin, uint baudrate) {
    ...
    dma_channel_configure(dma_channel_, &config_dma,
...
char uart_rx_read(void) {
    ...
    if (uart_rx_overflow()) buffer_pos_ = transfer_count_ - BUFFER_SIZE;
    char value = buffer_[buffer_pos_ % BUFFER_SIZE];
    if (buffer_pos_ < transfer_count_) buffer_pos_++;
    return value;
}
...
bool uart_rx_overflow(void) {
    ...
    if (transfer_count_ - buffer_pos_ > BUFFER_SIZE) return true;
    return false;
}
